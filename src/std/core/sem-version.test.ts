...
import { parseSemVersion, compareSemVersion } from './sem-version.ts';

Deno.test('Semversion parsing, should match expected', () => {
...
Deno.test('Semversion parsing, should ignore space', () => {
  const expected = {
    major: 1,
    minor: 18,
    patch: 2,
...
Deno.test('Semversion parsing, should throw error on invalid', () => {
  assertThrows(() => {
    parseSemVersion('potatos');
  });
});

Deno.test('Semversion comparison, should return expectd', () => {
