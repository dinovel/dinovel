...
Deno.test('Reactive Subject', async (ctx) => {
  await ctx.step('Should emit value', async () => {
  ...
  await ctx.step('Should emit error', async () => {
  ...
  await ctx.step('Should not emit after completed', async () => {
    const subject = new Subject<string>();
